...
let MatrixParserTests =
  testList "MatrixParserTests" [
    ...
    testCase "Parse with mxRowMany operator 3" <| fun _ ->
        let pcolors = pipe2 (pstring "COLOR") pint32 (sprintf "%s%d") |> (!^)
        let parser = !^pZip ^<==> mxRowMany (pcolors <==> pFraction)
        runMatrixParser parser workSheet
        |> List.ofSeq
        |> function 
            |   [
    ...
    testCase "Parse with mxRowMany operator 3" <| fun _ ->
        let pcolors = pipe2 (pstring "COLOR") pint32 (sprintf "%s%d") |> (!^)
        let parser = mxRowMany (pcolors <==> pFraction)
        runMatrixParser parser workSheet
        |> List.ofSeq
        |> function 
            |   [
